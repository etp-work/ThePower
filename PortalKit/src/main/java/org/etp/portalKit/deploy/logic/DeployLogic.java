...
/**
 * The purpose of this class is to provide a bussiness logic adapter for deploy
 * related function.
 */
...
public class DeployLogic {
	 ...
	 * all the .gz file's name will be returned. Otherwise, the specified type
	 * of the .gz file's name will be returned.
	 * 
	 * @param cmd
	 *            cmd.downloadPath path of the downloaded packages from CI.
	 *            cmd.typeToDeploy typeToDeploy
	 *            referencePortal/multiscreenPortal, those two types are
	 *            supported.
	 * @return array of PackageChoose that can be used to deploy.
	 */
	public PackageCheckedResult setPathAndRetrieveincludedPkgs(
			CheckPackageCommand cmd) {
		...
		File[] allDeployedPkgs = packageCheck.retrieveDeployedPkgs(cmd
				.getDownloadPath());
	...
	/**
	 * Find specified .gz files under specified downloaded path, firstly, unpack
	 * them. And scan the unpacked folders, find all the needed war files and
	 ...
	 *            cmd.deployPackages .gz file names that used to unpack and
	 *            deploy.
	...
	public boolean deployInType(CheckPackageCommand cmd) {
		...
		if (StringUtils.isBlank(downLoadPath)) {
			throw new RuntimeException(
					"downloadPath could not be empty or null.");
		}
		if (StringUtils.isBlank(type2Deploy)) {
			throw new RuntimeException(
					"typeToDeploy could not be empty or null.");
		}
		if (CollectionUtils.isEmpty(packages)) {
			throw new RuntimeException("packages could not be empty or null.");
		}

		File[] allDeployedPkgs = packageCheck
				.retrieveDeployedPkgs(downLoadPath);
		...
		for (File file : allDeployedPkgs) {
			...
			try {
				...
				if (!unGziped.isFile()) {
					throw new RuntimeException("UnGzip " + file.getName()
							+ " error.");
				}
				File unTared = CompressUtil.unTar(unGziped.getAbsolutePath());
				if (!unTared.isDirectory()) {
					throw new RuntimeException("UnTar " + unGziped.getName()
							+ " error.");
				}

			} catch (IOException e) {
				throw new RuntimeException("UnPack " + file.getName()
						+ " error.");
			}
		}
		...
		if (portals == null) {
			throw new RuntimeException(
					"Read relationship from deployInfo4CI.json error.");
		}
